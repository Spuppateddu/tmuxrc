// Report opencode's state to tmux, so name_windows.sh can paint a colored dot
// on the window tab. The mirror image of the Claude Code hooks that install.sh
...
export const TmuxStatus = async ({ $ }) => {
  // A status dot is never worth breaking the agent over: swallow every failure
  // (not in tmux, script missing, pane already gone).
  const mark = (state) => $`${AGENT_STATE} ${state}`.quiet().nothrow()

  return {
    "session.created": async () => { await mark("busy") },
    // Re-assert busy after each tool: that's what clears the yellow dot once
