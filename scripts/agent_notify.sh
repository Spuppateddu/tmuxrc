...
#       arrives about a minute after Stop, and mapping it to `wait` would flip a
#       finished [x] window back to [ ] while you weren't looking.
#
...
case "$input" in
  ...
  *"waiting for your input"*) state=done ;;
  ...
  # treated as needing you - a false [ ] is cheap, a missed one is not.
  *) state=wait ;;
esac
